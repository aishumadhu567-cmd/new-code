...
//   ChevronDown, Zap, BarChart3, FileText, Sun, LogOut
// } from "lucide-react";
...
// // ✅ ADD HERE (after showError function)

// const validateSearch = () => {
//   if (!searchForm.month || !searchForm.year) {
...
//       {/* ── HEADER (original, but now below top bar) ── */}
...
//   <div className="salary-card" ref={resultsRef}>
...
export default function HrSalaryManagement() {
  const [generateForm, setGenerateForm] = useState({ totalDay: "", actualDay: "", month: "", year: new Date().getFullYear().toString() });
  const [searchForm, setSearchForm] = useState({ month: "", year: "" });
  const [result, setResult] = useState(null);
  const [loading, setLoading] = useState(false);
  const [searchLoading, setSearchLoading] = useState(false);
...
//   }catch (err) {
    ...
    // // 🔥 If 400 or 500 → show salary already generated
    // if (status === 400 || status === 500 || message.toLowerCase().includes("already")) {
    //   alert("⚠️ Salary already generated for this month and year");
    // } else {
  ...
  const submitSalary = async () => {
    if (!generateForm.year || !generateForm.month) {
      alert("Please select year and month before generating salary");
      return;
    }
    setLoading(true);
  ...
  try {
    ...
    try {
      ...
      // 🔹 If API returns 400 → salary not generated yet
    ...
    const monthLabel =
      months.find((m) => m.value === Number(generateForm.month))?.label || "";

    if (salaryExists) {
      alert(`⚠️ Salary already generated for ${generateForm.year} ${monthLabel}`);
      showToast(`⚠️ Salary already generated for ${generateForm.year} ${monthLabel}`, "error");
      return;
    }
  ...
  const handleEdit = (record) => {
    ...
    setEditForm({
      ...
      month: record.month || searchForm.month,
      year: record.year || searchForm.year, 
  ...
  return (
    <div className="salary-management-container">
      {toast.message && (
        <div className={`salary-toast ${toast.type === "error" ? "error" : "success"}`}>
          {toast.message}
        </div>
      )}
      <div className="salary-header" style={{ marginBottom: 20 }}>
  ...
  .filter(m =>
    generateForm.year
      ? Number(generateForm.year) < currentYearValue ||
        (Number(generateForm.year) === currentYearValue && m.value < currentMonth)
      : true
  )
  .map(m => (
